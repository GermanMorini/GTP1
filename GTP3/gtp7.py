...
class Persona:
        ...
        def __init__(self, nombre:str, apellido:str, direccion:str, localidad:str, cod_pos:int, nacimiento:Fecha):
                self.nombre = nombre
        ...
        def __repr__(self) -> str:
                return f"nombre: {self.nombre}\napellido: {self.apellido}\ndireccion: {self.direccion}\nlocalidad: {self.localidad}\ncódigo postal: {self.cod_pos}\nnacimiento: {self.nacimiento}"

class Programa:
        opciones:dict
        personas:list[Persona]

        def __init__(self, personas:Persona):
                self.personas = personas
                self.opciones = {"1": self.consulta_apellido, "2": self.consulta_localidad, "3": self.consulta_nacimiento}
                
        def __algoritmo_de_busqueda(self, consulta:str, qry_type="apellido") -> list[Persona]:
                rtn = []

                for p in self.personas:
                        muestra = p.__getattribute__(qry_type)
                        encontrado:bool = 1

                        for i in range(len(muestra) - len(consulta)):
                                for j in range(i, len(muestra) - (len(consulta) - i)):
                                        encontrado *= (consulta[j] == muestra[j])

                                if encontrado: rtn.append(p)

                return rtn

        def consulta_apellido(self):
                consulta = input("Ingrese el apellido a buscar: ")
                result = self.__algoritmo_de_busqueda(consulta)
        ...
        def consulta_localidad(self):
                consulta = input("Ingrese una localidad: ")
                result = self.__algoritmo_de_busqueda(consulta, "localidad")
        ...
        def consulta_nacimiento(self):
                consulta = input("Ingrese la fecha de nacimiento (dd/mm/yyyy): ")
                result = self.__algoritmo_de_busqueda(consulta, "nacimiento")
